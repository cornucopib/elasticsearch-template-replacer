...
class EsQueryTemplateEngineV2 {
    ...
    /**
     * 完全匹配正则（预编译）
     * 整个字符串恰好是一个变量占位符 {varName}
     *
     * 【与 VAR_PATTERN 的分工】
     * - EXACT_VAR_PATTERN：判断是否"完全匹配"，用于类型安全替换（保持原始类型）
     * - VAR_PATTERN：提取所有变量、字符串插值替换（多变量场景）
     */
    ...
    static Object deepClone(Object obj) {
        ...
        if (obj instanceof Map) {
            ...
            for (Map.Entry<?, ?> entry : source.entrySet()) {
                result.put(entry.getKey(), deepClone(entry.getValue()))
            }
            return result
        }
        ...
        if (obj instanceof List) {
            ...
            for (int i = 0; i < source.size(); i++) {
                result.add(deepClone(source.get(i)))
            }
            return result
        }
    ...
    private static void resolveAndCleanMap(Map source, Map<String, Object> params,
        ...
        while (iter.hasNext()) {
            ...
            if (!entryRemoved) {
                 ...
                 * - 省去 2 次 Matcher 创建和 2 次正则匹配（EXACT + VAR）
                 */
                if (key.indexOf('{') >= 0) {
                    ...
                    if (resolvedKey.is(REMOVE_SENTINEL)) {
                        ...
                        // key 发生了变化，需要替换
                        if (keyReplacements == null) keyReplacements = new LinkedHashMap<>()
     ...
     *
     * 第二段：快速判断无变量 → 大部分字符串走这条路径
     *   - 检查字符串是否包含任何 {var} 模式
     *   - 如果没有，直接返回原字符串，省去后续处理
     *   - 例："hello world" → 直接返回
     *
     * 第三段：部分匹配 → 字符串插值
     *   - 字符串包含变量但不是完全匹配，如 "prefix_{var}_suffix"
     *   - 必须进行字符串拼接，结果一定是 String
     *
     * @param str 待解析的字符串
     * @param params 变量参数
     * @param strict 是否严格模式
    ...
    private static Object resolveString(String str, Map<String, Object> params, boolean strict) {
        ...
        if (!varMatcher.find()) {
            return str  // 不含任何变量，直接返回
        }

        // === 第三段：部分匹配，字符串插值 ===
        varMatcher.reset()  // 重置 Matcher 位置，从头开始替换
        ...
        while (varMatcher.find()) {
            ...
            if (params.containsKey(varName)) {
                ...
                anyMissing = true
                // 宽松模式下保留原始占位符（部分变量存在时）
                replacement = varMatcher.group(0)
            }

            // Matcher.quoteReplacement 转义 $ 和 \ 等特殊字符
