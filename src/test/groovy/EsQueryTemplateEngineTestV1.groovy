 ...
 */
class EsQueryTemplateEngineTestV1 {
    ...
    static void testMatchQuery() {
        ...
        def result = EsQueryTemplateEngineV1.resolve(template, params)
    ...
    static void testStrictModeException() {
        ...
        try {
            EsQueryTemplateEngineV1.resolveStrict(template, params)
    ...
    static void testExtractVariables() {
        ...
        '''
        def variables = EsQueryTemplateEngineV1.extractVariables(template)
    ...
    static void testValidate() {
        ...
        def missing = EsQueryTemplateEngineV1.validate(template, params)
    ...
    static void testResolveToMap() {
        ...
        def result = EsQueryTemplateEngineV1.resolveToMap(template, params)
    ...
    static void testDeepClone() {
        ...
        def cloned = EsQueryTemplateEngineV1.deepClone(original)
    ...
    static void testDeepNesting() {
        ...
        def result = EsQueryTemplateEngineV1.resolve(json.toString(), [value: "deep"])
    ...
    static void testDepthLimitExceeded() {
        ...
        try {
            EsQueryTemplateEngineV1.resolve(json.toString(), [v: "test"])
    ...
    static void testInvalidParamsType() {
        ...
        try {
            ...
            EsQueryTemplateEngineV1.resolve(template, invalidParams as Map)
    ...
    static void testArrayTemplateJson() {
        try {
            EsQueryTemplateEngineV1.resolve('[1, 2, 3]', [:])
            assert false : "应该抛出类型异常"
        } catch (IllegalArgumentException e) {
    ...
    static void testSpecialCharVariableNames() {
        ...
        def result1 = EsQueryTemplateEngineV1.resolve(template1, ["user-id": "U001"])
        def json1 = JSON.parse(result1)
        assert json1.query.term["user-id"] == "U001"

        // 点号变量名
        def template2 = '{"query": {"term": {"{field.name}": "{field.value}"}}}'
        def result2 = EsQueryTemplateEngineV1.resolve(template2, ["field.name": "status", "field.value": "active"])
        def json2 = JSON.parse(result2)
        assert json2.query.term.status == "active"

        // 混合变量名
        def template3 = '{"filter": "{my-filter.type}"}'
        def result3 = EsQueryTemplateEngineV1.resolve(template3, ["my-filter.type": "range"])
    ...
    static void testNullParamValue() {
        ...
        def result = EsQueryTemplateEngineV1.resolve(template, [statusCode: null, name: "test"])
        def json = JSON.parse(result)
    ...
    static void testExtractVariablesWithSpecialChars() {
        ...
        def vars = EsQueryTemplateEngineV1.extractVariables(template)
